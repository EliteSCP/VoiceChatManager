...
namespace VoiceChatManager.Core.Audio.Capture
{
    ...
    public class VoiceChatCapture : IVoiceChatCapture
    {
        ...
        public VoiceChatCapture(WaveFormat waveFormat, int readBufferSize, int readInterval, ILog log = null)
        {
        ...
        /// <inheritdoc/>
        public ConcurrentDictionary<ITalker, IVoiceChatRecorder> Recorders { get; } = new ConcurrentDictionary<ITalker, IVoiceChatRecorder>();
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                await Recorders.ParallelForEachAsync(
                    async recorder =>
                    {
                        if (!recorder.Key.PlayBackComponent.HasActiveSession)
                            return;
                        ...
                        try
                        {
                            recorder.Key.PlayBackComponent.OnAudioFilterRead(samples, 1);
                        ...
                        {
                            log?.Error($"[{typeof(VoiceChatCapture).FullName}.{nameof(StartAsync)}] Cannot write voice chat samples of {recorder.Value.Talker?.Nickname} ({recorder.Value.Talker?.Nickname})!\n{exception}");
                        }
                        finally
                        {
                            if (!recorder.Key.PlayBackComponent.HasActiveSession)
